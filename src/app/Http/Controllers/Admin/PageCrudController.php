...
class PageCrudController extends CrudController
{
    ...
    protected function setupListOperation()
    {
        $this->crud->addColumn([
            'tab'   => _t('Content'),
            'name' => 'name',
            'label' => _t('pagemanager.name'),
        ]);
        $this->crud->addColumn([
            'tab'   => _t('Content'),
            'name' => 'template',
            'label' => _t('pagemanager.template'),
            'type' => 'model_function',
            'function_name' => 'getTemplateName',
        ]);
        $this->crud->addColumn([
            'name' => 'slug',
            'label' => _t('pagemanager.slug'),
        ]);
    ...
    public function addDefaultPageFields($template = false)
    {
        ...
        $this->crud->addField([
            'tab'   => _t('Main info'),
            'name' => 'name',
            'label' => _t('pagemanager.page_name'),
            'type' => 'text',
            'wrapperAttributes' => [
                'class' => 'form-group col-md-6',
            ],
        ...
        $this->crud->addField([
            'tab'   => _t('Main info'),
            'name' => 'title',
            'label' => _t('pagemanager.page_title'),
            'type' => 'text',
            // 'disabled' => 'disabled'
        ]);
        $this->crud->addField([
            'tab'   => _t('Main info'),
            'name' => 'slug',
            'label' => _t('pagemanager.page_slug'),
            'type' => 'text',
            'hint' => _t('pagemanager.page_slug_hint'),
            // 'disabled' => 'disabled'
        ]);
    }
    ...
    public function getTemplatesArray()
    {
        ...
        foreach ($templates as $template) {
            $templates_array[$template->name] = _t(str_replace('_', ' ', Str::title($template->name)));
        }
